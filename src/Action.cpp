...
void SimulateStep::act(WareHouse &wareHouse) {
    ...
    for(int i=1; i<=numOfSteps;i++){ //performing a certain number of steps
        ...
        for (size_t index = 0; index < pending.size(); index ++){// we are deleting while iterating so using an iterator messes things up
        ...
        //stages 2 - 4
        for (size_t index = 0; index < volunteers.size() ; index ++){//for(Volunteer* v: volunteers){ //iterate all volunteers
...
void PrintCustomerStatus::act(WareHouse &wareHouse){
    if(!wareHouse.customerExists(customerId)){ //check if customer exists
        ...
        std::cout << "numOrdersLeft: " + to_string(wareHouse.getCustomer(customerId).getOrdersLeft()) << endl;
    }
}
